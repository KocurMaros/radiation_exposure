#!/bin/bash
# filepath: /home/user/Projects/radiation_drone/jetson_remote_logger_via_rpi.sh
# Logs Jetson Xavier data through Raspberry Pi proxy to local PC
...
echo "--- Jetson Remote Logger via Raspberry Pi ---"
echo "PC IP: 192.168.88.14"
echo "Raspberry Pi: ${RPI_USER}@${RPI_IP}"
echo "Jetson (via RPI): ${JETSON_USER}@${JETSON_IP}"
...
echo -n "  - Testing PC -> Raspberry Pi... "
if ssh -o ConnectTimeout=5 "${RPI_USER}@${RPI_IP}" "echo OK" &>/dev/null; then
    echo "OK"
else
    echo "FAILED"
    echo "ERROR: Cannot connect to Raspberry Pi. Check network and SSH keys."
    exit 1
fi

echo -n "  - Testing Raspberry Pi -> Jetson... "
if ssh -o ConnectTimeout=5 "${RPI_USER}@${RPI_IP}" "ssh -o ConnectTimeout=5 ${JETSON_USER}@${JETSON_IP} 'echo OK'" &>/dev/null; then
    echo "OK"
else
    echo "FAILED"
    echo "ERROR: Raspberry Pi cannot connect to Jetson."
    echo "Make sure:"
    echo "  1. Raspberry Pi is connected to Jetson WiFi (10.42.0.x)"
    echo "  2. SSH keys are set up between RPI and Jetson"
    echo "  3. Run on RPI: ssh-copy-id ${JETSON_USER}@${JETSON_IP}"
    exit 1
fi

# --- 1. Execute One-Time Dumps ---
echo -e "\n1. Capturing static boot logs..."
ssh "${RPI_USER}@${RPI_IP}" "ssh ${JETSON_USER}@${JETSON_IP} \"${ONE_TIME_CMD}\"" > "${LOG_DIR}/initial_logs.txt" 2>&1
if [ $? -ne 0 ]; then
...
ssh "${RPI_USER}@${RPI_IP}" "ssh ${JETSON_USER}@${JETSON_IP} \"${TEGRASTAT_CMD}\"" > "${LOG_DIR}/tegrastats_continuous.log" 2>&1 &
TEGRASTAT_PID=$!
...
ssh "${RPI_USER}@${RPI_IP}" "ssh ${JETSON_USER}@${JETSON_IP} \"${DMESG_CMD}\"" > "${LOG_DIR}/dmesg_continuous.log" 2>&1 &
DMESG_PID=$!
BG_PIDS+=("$DMESG_PID")
...
(
  while true; do
    ...
    ssh "${RPI_USER}@${RPI_IP}" "ssh ${JETSON_USER}@${JETSON_IP} 'sudo journalctl --no-pager --lines=100'" > "${OUTFILE}" 2>&1
...
# Copy script to Jetson via RPI
scp -q mem_checksum_guard.py "${RPI_USER}@${RPI_IP}:/tmp/mem_checksum_guard.py"
ssh "${RPI_USER}@${RPI_IP}" "scp -q /tmp/mem_checksum_guard.py ${JETSON_USER}@${JETSON_IP}:/tmp/"

# Run the script on Jetson
ssh "${RPI_USER}@${RPI_IP}" "ssh ${JETSON_USER}@${JETSON_IP} 'export MEMCHK_SIZE_MB=${MEMCHK_SIZE_MB} MEMCHK_ALLOC_INTERVAL=${MEMCHK_ALLOC_INTERVAL} MEMCHK_CHECK_INTERVAL=${MEMCHK_CHECK_INTERVAL} MEMCHK_MIN_FREE_PCT=${MEMCHK_MIN_FREE_PCT}; python3 /tmp/mem_checksum_guard.py'" > "${LOG_DIR}/mem_checksum.log" 2>&1 &
MEMCHK_PID=$!
BG_PIDS+=("$MEMCHK_PID")
